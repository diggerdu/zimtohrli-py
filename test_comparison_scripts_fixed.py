...
try:
    # Import the C++ extension directly and wrap it
    ...
    # Create a zimtohrli-like interface for the test
    class zimtohrli:
        @staticmethod
        def compare_audio(audio_a, sample_rate_a, audio_b, sample_rate_b, return_distance=False):
        ...
        @staticmethod
        def load_and_compare_audio_files(file_a, file_b, return_distance=False):
            if not SOUNDFILE_AVAILABLE:
                raise ImportError("soundfile required for file-based comparison")
            
            audio_a, sr_a = sf.read(file_a)
            audio_b, sr_b = sf.read(file_b)
            
            return zimtohrli.compare_audio(audio_a, sr_a, audio_b, sr_b, return_distance)
        
        @staticmethod
        def zimtohrli_distance_to_mos(distance):
            return _mos_from_zimtohrli(float(distance))
        
        @staticmethod
        def get_expected_sample_rate():
            return 48000
        
        @staticmethod
        def ZimtohrliComparator():
            return ZimtohrliComparatorWrapper()
    
    class ZimtohrliComparatorWrapper:
        def __init__(self):
            self._zimtohrli = _ZimtohrliCore()
        
        def compare(self, audio_a, audio_b, return_distance=False):
            # Convert to float32 if needed
            if audio_a.dtype != np.float32:
                audio_a = audio_a.astype(np.float32)
            if audio_b.dtype != np.float32:
            ...
            if return_distance:
                ...
                return zimtohrli.zimtohrli_distance_to_mos(distance)
